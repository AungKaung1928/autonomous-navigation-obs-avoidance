...
class SimpleObstacleAvoider(Node):
    def __init__(self):
        ...
        # Speed settings - reduced for smooth movement
        self.normal_speed = 0.7 # Changed from 0.3
        self.turn_speed = 0.3

        self.get_logger().info('Simple Obstacle Avoider started!')

    def laser_callback(self, msg):
        """Process laser scan data to detect obstacled and measure lane widths"""
        try:
            ...
            # Filter out invalid reading (inf, nan, 0)
            ...
            # Measure lane widhts before entering the lane
    ...
    def move_robot(self):
        """Main movenment logic - think ahead and avoid narrow lanes"""
        cmd = Twist()
        ...
        try:
            if self.front_clear:
                ...
                # Check if lanes are wide enough to enter
                ...
                if right_safe and left_safe:
                    # if both lanes are safe, choose wider lane, default to left if equal
                    ...
                    if self.right_clear or self.left_clear:
                        self.get_logger().info('Lane too narrow - continuing to search for wider path')
                    else:
